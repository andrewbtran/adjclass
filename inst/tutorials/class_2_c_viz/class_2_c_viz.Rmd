...
We'll use the annual disasters this time so we hve more than just the fires.
...
```{r facet_example1-solution, exercise=TRUE}
annual_disasters %>% 
  filter(incidentType %in% c("Hurricane", "Fire", "Flood")) %>% 
ggplot() +
  geom_col(mapping=aes(x= year, y= total)) +
...
Now we can add more customizations to the chart.
...
Just separate the arguments with commas.
```
...
See you next time when we start trying to use R to obtain difficult data.
