...
<iframe width="560" height="315" src="https://www.youtube.com/embed/_Gc_4NryOhw" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
The  `/v1/combined_county_annual` is the type of data we're calling from the api. 
...
The `key=WaPo` is the authorization token which tells the Washington Post server that I'm allowed to access the data.
...
The jsonlite package just transformed all the squiggles and commas from what you saw in the browser output and gave you a clean looking data frame.
...
But a lot of basic APIs will give you this level of data that's simple to parse and transform into a data frame for analysis.

## Prep for looping

What if we wanted multiple counties of data?
...
The function `URLencode()` from the **utils** package should set this up for us.
...
Now, go forth and apply the same methodology to other [APIs](https://apilist.fun/)! Congress.gov just recently unveiled a new [legislative API](https://api.congress.gov/) that could be useful to reporters.
...
**Warning:** Not all JSON is similar. It can get pretty gnarly and complicated, as I've complained about before. If you ever find yourself dealing with some oddly structured JSON, there are [good methods](https://jennybc.github.io/purrr-tutorial/) to get around it but it's going to involve some custom parsing.
