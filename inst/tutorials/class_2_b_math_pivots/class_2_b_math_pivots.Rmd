...
And in the 10th line, add "state" as a second `group_by()` option after "year".
...
Okay, we have tidy data! Each variable is in its own column. Each case is in its own row and each valueis in its own cell.

This makes it easier to mutate and manipulate (and is also the preferable data structure for most data viz tools).

However, if we wanted to compare 2020 declaration counts to 2020 in each state, that would be difficult.
...
Kentucky had 50 in 2021 and 14 in 2022. 
...
The formula for percent change is (new-old)/old*100.
```
...
When you're ready to move on to the final part of module 2, just type this in the console of RStudio:

```
learnr::run_tutorial("class_2_c_viz", "adjclass")
```
