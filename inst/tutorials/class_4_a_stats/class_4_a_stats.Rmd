...
candles <- read_csv("https://www.andrewbatran.com/data/scented.csv")
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/vFzXONpVO7E" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
Some of the most common basic descriptive stats you can do are calculating mean and median.

Mean (or average) equals the sum of the values then divided by how many values there were. So (4+4+7+7+8+9+10+10+11+100)/14 or 170/14=17.

The median is the middle number in that list, so it'd be between 8 and 9, so 8.5. Meidan is useful to deal with outliers, for example.
...
*Fix line 7, 8, 9, and 11, please.*

Note, be sure to use `ungroup()` in line 8 otherwise the sorting will only occur in the individual months and not as a whole.
...
```{r weekly_am1-hint}
What's the if else statement function in mutate?

If there are no complaints about smell then fill no_scent with the opposite of 1? (It's not 2).
...
I'll go ahead and do that for you and save it as "covid_long_monthly".
...
I'll do that for you now and save it as "monthly_wide".
...
This way you can create cross tabs and get around having to use correlations in your narrative.
...
I've saved it and deleted identifying information as **boston_salaries** (if you insist on working with the raw data, it's not hard to find via search):
...
For example, this is an example of the Datasaurus data (created by [Alberto Cairo](https://albertocairo.com/)) with the same statistical properties when it comes to mean, median, standard deviation, and correlation figures. But the underlying data are all completely different.
...
Let's work with some fake data on employee salaries so we can investigate whether or not there is a gender pay gap at this company. 
...
*Note: I just realized "be careful, this could be misleading" is like the data journalism unofficial motto.*
...
Let's begin by bringing in the data.

I've done for you already below. Take a look at what the data includes:
...
The time creating models is like 85% cleaning and prepping the data.

So we'll continue doing so by adding a new numerical variable to change Male to 1 and Female to 0. 

I'll go ahead and do that for you below:
...
I'll go ahead and run this code for you and save it as "unadjusted_jobs".
...
Looks like men are very over represented in management and software roles, which are more highly paying, while women seem over represented among marketing associate roles. 
...
Finally, let's run a model with all of the controls that you have access to, including job
...
Importing, tidying, and then iterating between transforming the data, visualizing, and modeling it.

Until you get enough to communicate what's important!
